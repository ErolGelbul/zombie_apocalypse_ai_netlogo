    ...
    ifelse brains = true [                           ;if we have seen a human following events will be executed
     ...
     ask humans in-radius rad [                      ;detect if a zombie engaged with a human(was able to catch up)
        ...
        ifelse chance >= 50 [                        ;if the chance calculated from courage is above 50 percent
              ask predator [die]                     ;kill the zombie
...
@#$#@#$#@
## 1. WHAT IS THE ZOMBIE APOCALYPSE MODEL ABOUT?

The aim of the model is to understand the spread of the disease, Zombiism, in a large number of people within a short period of time.

## 2. MAIN OBJECTIVES OF THE AGENTS

There are two agents in this model, Zombies and Humans. Zombies are seeking humans to turn into Zombies and essentially spread the virus. Humans are trying to monitor their surroundings and defend the human kind.

## 3. HOW TO USE THE MODEL

The model comes with 5 monitoring elements to identify real-time changes, such as: Humans, Zombies, Humans Turned, Zombies Killed and Engagements. Additionally, a plot for population tracking.

For the initial trial, it is recommended that the settings are not altered, because current settings are prefered to be an optimal starter degree to visualize the model.

After changing values (section 4), press "Setup" and then "Go" to start simulation.


## 4. CHANGING VALUES

1) Courage
After inspecting the model with a couple of simulations run beforehand, adjusting "human_courage" would affect ultimate result. If the percentage is increased, the humans are more likely to stop fight. Moreover, their chance of winning the encounter against a zombie is increased. On the other hand, if the percentage is decreased, the outcome of an encounter is more likely to result with the zombie, turning the human into a zombie.

2) Number of Humans and Zombies
Altering these values will might chance the outcome of the simulation, however the result vastly relies on Courage percentage. However, difference in these values will affect the speed and time of the events happenning. For instance, more zombies would lead to a quicker victory for zombies.

3) Speed of Humans and Zombies
These values apply similar affects to the simulation as in agent count, mentioned above. For instance, increasing zombies' speed would result more encounters, and depending on the courage factor.

4) Show Vision Cone for Humans and Zombies
When these are turned on, it helps the user to visualize the vision range for the agents.

## CREDITS

Daniel Olofin & Erol Gelbul
@#$#@#$#@
default
true
0
