...
humans-own [ zombie_encounters zombies_engaged ]     ; this will create 2 variables which will be used to count the total zombies seen (encountered) and total engagements with a zombie

globals [rad]                                        ; a global variable which is radius (used as the radius for interactions)
...
to initiate_zombies
  ask zombies [
    ...
    ifelse brains = true [                           ;if we have seen a human following events will be executed
      ask zombies [
        let candidates humans in-radius 18           ; observe the candidate human around our 18 radius of our cone
        if any? candidates [                         ; if there are any candidates, let's target the closest one
          let target min-one-of humans [distance myself]
          face target                                ; turn our face towards the target
...
to initiate_humans
  ask humans [
    ...
    ;;if/else statement
    ifelse danger = true [
      set zombie_encounters zombie_encounters + 1     ; add one to the zombie_encounters counter
